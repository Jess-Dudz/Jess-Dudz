# Hi, I'm Jessi! 👋

### 📊 Data Scientist | MBA | Operational Strategist
I specialize in bridging the gap between high-level leadership and technical analytics. Currently pursuing an **MS in Data Science**, I am focused on applying stochastic modeling and behavioral economics to solve complex operational challenges.

---

### 🛠️ Technical Toolkit
* **Languages:** Python (NumPy, Pandas, Matplotlib, Scikit-Learn), SQL, R
* **Specializations:** Stochastic Modeling, Predictive Analytics, Strategic Planning
* **Business Intelligence:** Data Visualization, Operational Workflow Optimization
* **The Narrative Edge:** Professional storytelling and technical communication (MA in English)

---

### 🚀 Portfolio in Development
*I am currently building out a suite of independent research projects. While the source code for these projects is currently in private development, summaries of my methodologies are below:*

**🌪️ The Great Migration & The Nocturnal Creep**
* **Status:** Complete (Technical Analysis & ML Modeling)
* **The Project:** A spatiotemporal study of 75 years of NOAA records investigating the geographic shift of tornado density toward the Southeast and the "nocturnal creep" of nighttime touchdowns. 
* **The Tech:** Built a high-performance **DuckDB** ETL pipeline to process 43,000+ records and deployed a **SMOTE-enhanced Random Forest Classifier** to mathematically model nocturnal risk factors.
* **The Variance Engine**
    > **Status:** *Longitudinal Study (Phase 1: Data Acquisition)*
    > A predictive model analyzing the ROI of loyalty programs across land-based and maritime gaming environments. This project utilizes stochastic modeling to validate variance-based reinvestment theories over a year-long study period.
* **Carnival Algorithm Decoder**
    > **Status:** *Technical Analysis (Complete)*
    > An independent deep-dive into gaming patterns and algorithmic outcomes. This research focuses on decoding reinvestment strategies to understand how data-driven incentives influence player behavior.

---

### 🎰 Research Focus
I operate at the intersection of **behavioral economics** and **predictive analytics**. I am particularly interested in how data-driven incentives—such as loyalty programs and reinvestment strategies—drive customer behavior in high-stakes, high-variance industries.

---

### 🖋️ Beyond the Data
When I'm not in a Jupyter Notebook, you can find me:
* **Writing:** Leveraging my background in linguistics for technical documentation and narrative storytelling. I am a published author with a passion for clear, impactful communication.
* **Exploring:** Planning my next 10-day cruise or working toward a scuba certification.
* **Gaming:** Navigating complex systems and team-based quests in *World of Warcraft*.

---

### 📫 Connect with me
[**LinkedIn**](https://www.linkedin.com/in/jessi-dudz/) | [**Email**](mailto:jessdudz78@outlook.com)

---
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Jess-Dudz&show_icons=true&theme=radical" alt="Jessi's GitHub Stats" />
</p>
